...
impl VirDomain {
    ...
    pub fn interface_stat(self, name: &str) -> Result<VirDomainInterfaceStats, VirError> {
        unsafe {
            ...
            let size = mem::size_of::<virt::virDomainInterfaceStatsStruct>();
            let stats: virt::virDomainInterfaceStatsPtr = libc::malloc(size) as virt::virDomainInterfaceStatsPtr;
            let results = virt::virDomainInterfaceStats(self.ptr, path.as_ptr(), stats, size as u64);
            if results != -1 {
